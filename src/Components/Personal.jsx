...
const Daily = () => {
    ...
    const tags = ['Fitness', 'Work', 'Rest', 'Gaming', 'News'];
    ...
    const Submit = () => {
        ...
        const storedPersonals = localStorage.getItem('Personal');
        if (storedPersonals) {
            const PersonalsArray = JSON.parse(storedPersonals);
            setCount(PersonalsArray.length);
        }
    }

    let Saved = JSON.parse(localStorage.getItem('Personal'));
    let completedPersonal = JSON.parse(localStorage.getItem('Personalcomplete'))
    useEffect(() => {
        if (Saved) {
            setAllData(Saved);
    ...
    const handledelete = index => {
        ...
        localStorage.setItem('Personal', JSON.stringify(reduceData))
    ...
    const handleComplete = index => {
        ...
        handledelete(index)
        localStorage.setItem('Personalcomplete', JSON.stringify(updatedData))
    }
    ...
    const handleCompletedelete = (index) => {
        ...
        localStorage.setItem('Personalcomplete', JSON.stringify(reduceData))
    ...
    return (
        <>
            <div className='ms-60'>
                ...
                <h1 className=' text-center text-4xl p-4 tracking-wider uppercase font-["SF_Pro_Rounded"] font-bold'>Personal</h1>
