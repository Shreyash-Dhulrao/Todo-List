...
const Special = () => {
    ...
    const Submit = () => {
        ...
        setAllData(newData);
        localStorage.setItem('Special', JSON.stringify(newData))

        setTask('')
        setDescription('')

        const storedSpecials = localStorage.getItem('Special');
        if (storedSpecials) {
            const SpecialsArray = JSON.parse(storedSpecials);
            setCount(SpecialsArray.length);
        }
    }

    let Saved = JSON.parse(localStorage.getItem('Special'));
    let completedTask = JSON.parse(localStorage.getItem('Specialcomplete'));
    useEffect(() => {
        if (Saved) {
            setAllData(Saved);
    ...
    const handledelete = index => {
        ...
        localStorage.setItem('Special', JSON.stringify(reduceData))
    ...
    return (
        <>
            <div className='ms-60'>
                ...
                <h1 className=' text-center text-4xl p-4 tracking-wider uppercase font-["SF_Pro_Rounded"] font-bold'>Special Tasks</h1>
