...
const Monthly = () => {
    ...
    const Submit = () => {
        ...
        setAllData(newData);
        localStorage.setItem('TodoMon', JSON.stringify(newData))

        setTask('')
        setDescription('')
        setDate(ymd)

        const storedTodoMons = localStorage.getItem('TodoMon');
        if (storedTodoMons) {
            const TodoMonsArray = JSON.parse(storedTodoMons);
            setCount(TodoMonsArray.length);
        }
    }
    
    let Saved = JSON.parse(localStorage.getItem('TodoMon'));
    let completedTodo = JSON.parse(localStorage.getItem('TodoMonComplete'))
    useEffect(() => {
        if (Saved) {
            setAllData(Saved);
    ...
    const handledelete = index => {
        ...
        localStorage.setItem('TodoMon', JSON.stringify(reduceData))
    ...
    const handleComplete = index => {
        ...
        handledelete(index)
        localStorage.setItem('TodoMonComplete', JSON.stringify(updatedData))
    }
    ...
    const handleCompletedelete =(index)=>{
        ...
        localStorage.setItem('TodoMonComplete', JSON.stringify(reduceData))
    ...
    return (
        <>
            <div className='ms-60 '>
                ...
                <h1 className=' text-center text-4xl p-4 tracking-wider uppercase font-["SF_Pro_Rounded"] font-bold'>Monthly Tasks</h1>
